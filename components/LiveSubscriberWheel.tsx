...
export default function LiveSubscriberWheel() {
  ...
  return (
    <div className="flex flex-col md:flex-row-reverse gap-6 md:items-center">
      {/* Right: wheel — fills the box height */}
      <div className="md:flex-1 w-full flex items-center justify-center">
        <div className="w-full max-w-[460px]">
          <SpinWheel controlledItems={entries} emptyHint="" winWord="Winner" />
        </div>
      </div>
      ...
      {/* Heading */}
      <div className="mb-12">
        <span className="inline-block text-sm font-black uppercase tracking-[0.3em] text-purple-400 mb-2">Step 1</span>
        <h3 className="text-3xl sm:text-4xl md:text-5xl font-black uppercase text-white leading-tight">Giveaway Picker</h3>
        <p className="text-gray-400 text-base sm:text-lg mt-2">Connect to live chat — subscribers enter by keyword</p>
      </div>
      {/* Connection panel */}
      <div className="rounded-xl border border-purple-900/40 bg-[#07050f]/60 p-5 space-y-4 w-96 max-w-full mx-auto">
        <div>
          ...
          <input
            ...
            placeholder="!buck"
            className="w-full bg-[#0d0a1a] border border-purple-700/50 rounded-lg px-4 py-3 text-base text-white focus:outline-none focus:border-[#00ff87] disabled:opacity-50"
          />
        </div>
        ...
        <div className="space-y-2.5">
          {!isConnected ? (
            <button
              ...
              className="w-full text-black font-black py-3 px-5 rounded-lg uppercase tracking-widest text-sm transition-all disabled:opacity-50"
            ...
            <button
              onClick={disconnect}
              className="w-full border border-red-500/40 text-red-300 hover:bg-red-500/10 font-bold py-3 px-5 rounded-lg uppercase tracking-widest text-sm transition-all"
            >
              Disconnect
            </button>
          )}
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            {entries.length > 0 && (
              <button
                ...
                className="inline-flex items-center gap-1.5 border border-red-500/50 text-red-300 hover:bg-red-500/15 hover:text-red-200 font-bold py-1.5 px-3 rounded-lg uppercase tracking-widest text-[10px] transition-all"
              >
        ...
        {isConnected && (
          <p className="text-gray-600 text-xs">
            Subscribers type <span className="text-[#00ff87] font-bold">{keyword || 'any message'}</span> in chat to enter.
          </p>
        )}
      </div>

      {/* Entrants — opens a popup with the full list (read-only) */}
      <div className="w-full mt-4 flex justify-center">
        <button
          onClick={() => setShowEntrants(true)}
          className="inline-flex items-center gap-2 border border-purple-500/40 text-purple-200 hover:text-white hover:border-purple-400 hover:bg-purple-500/10 font-bold py-2.5 px-5 rounded-lg uppercase tracking-widest text-xs transition-all"
        >
